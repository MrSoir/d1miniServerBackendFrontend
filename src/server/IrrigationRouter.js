...
IrrigationRouter.setServerIp = (serverIp)=>{
	if( !serverIp ){
		console.log('IrrigationRouter.setServerIp - serverIp invalid!!!');
		return;
	}
	
	let serverIpChanged = false;
	if( !!serverIp.ipv4 && serverIp.ivp4 !== SERVER_IP ){
		SERVER_IP = serverIp.ipv4;
		serverIpChanged = true;
	}
	if( !!serverIp.port && serverIp.port !== SERVER_PORT){
		SERVER_PORT  = serverIp.port;
		serverIpChanged = true;
	}
	
	if( serverIpChanged ){
		updateServerIpToFrontendClients();
	}
};

function genServerIpData(){
	return {
		ipv4: SERVER_IP,
		port: SERVER_PORT
	}
...
function updateServerIpToFrontendClients(){
	sendUpdatedDataToAllConnectedSockets('updateServerIp', genServerIpData());
}
...
IrrigationRouter.setSocketIO = (socketIO)=>{
	...
	socketIO.on('connection', socket=>{
		...
		Object.assign(serverData, genServerIpData()); // merge ipv4- and port-attributes to serverData
		
		socket.emit('connectionEstablished', serverData);
	});
};
...
IrrigationRouter.route('/getServerIp').get((req,srcResp)=>{
	srcResp.status(200).send( genServerIpData() );
});
