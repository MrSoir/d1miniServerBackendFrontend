...
IrrigationRouter.setSocketIO = (socketIO)=>{
	...
	socketIO.on('connection', socket=>{
		console.log('\nSERVER: socket-io: client connected!');
		let id = 0;
		socket.on('setId', data=>{
			console.log('SERVER: socket-io: setId: ', data);
			console.log('socket.data: ', data);
			id = data.id;
			connectionIDs.set(id, socket);
		});
...
IrrigationRouter.route('/getIrrigationPlan').get((req,res)=>{
...
function getSocketId(req){
	return (!!req && !!req.query.socketId) ? req.query.socketId : null;
}
...
function setIrrigationPlan(dataObj, callback, req){
	...
	saveIrrigationPlanToFile(stdrdzdIrrigationPlan, (err)=>{
		if(err){
			...
			if(socketId){
				console.log('connectionIDs.has(socketId): ', connectionIDs.has(socketId));
				if(connectionIDs.has(socketId)){
					console.log('send updated irrigation-plan via socket');
					connectionIDs.get(socketId).emit('planUpdated', stdrdzdIrrigationPlan);
				}
			}
		}
	}, req);
...
function sendIrrigationPlanToArduino(irrigationPlan, res){
	...
	arduinoPlanStr = '';
	irrigationPlan.irrigationEntries.forEach(ie=>{
		arduinoPlanStr += ie.getUnformattedArduinoString();
	});
...
function loadIrrigationPlanFromFile(callback, req){
	...
	console.log('loadIrrigationPlanFromFile: tarFilePath: ', tarFilePath);
